...
midiPort = mido.open_output("TiMidity port 0")
banks = [88, 88, 1, 1, 1, 1, 1, 1]
notes = [
    [48, False], [50, False], [52, False], [55, False], [57, False], [60, False], [62, False], [64, False],
    [67, False], [69, False], [72, False], [74, False], [76, False], [79, False], [81, False], [84, False]
    ]

def playNotes(buffer):
    buffer = buffer[1:]
    for i in range(1, len(buffer) / 2, 2):
        chord = format(buffer[i], "08b") + format(buffer[i + 1], "08b")
        midiPort.send(mido.Message("program_change", program=banks[int((i - 1) / 2)], channel=int((i + 1) / 2)))
        for c in range(16): #len(chord)
            if chord[c] == '0' and notes[i][1] == False:
                midiPort.send(mido.Message("note_on", note=notes[c][0]))
                notes[i][1] = True
            elif chord[c] == '1' and notes[i][1] == True:
                midiPort.send(mido.Message("note_off", note=notes[c][0]))
                notes[i][1] = False
...
def send_frames(index, amount):
    ...
    if TASLINK_CONNECTED == 1:
        try:
            ...
            for i in range(amount):
                playNotes(inputBuffers[index][framecount + i])
        except IndexError:
    ...
    #send inputBuffers[index][framecount:(framecount + amount)]
