...
midiPort = mido.open_output("OSoTASPort")
    ...
    for i in range(1):
        ...
        for c in range(16): #len(chord)
            if chord[c] == '0' and notes[c][1] == False:
                midiPort.send(mido.Message("note_on", note=notes[c][0], channel=int((i + 1) / 2)))
                notes[c][1] = True
            elif chord[c] == '1' and notes[c][1] == True:
                midiPort.send(mido.Message("note_off", note=notes[c][0], channel=int((i + 1) / 2)))
                notes[c][1] = False
